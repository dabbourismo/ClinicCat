...
namespace ClinicCat.FrontEnd.Categories
{
    public static class CategoriesLogic
    {
        ...
        public static void DeleteButton(DataGridView dgv)
        {
            ...
            {
                int Row = dgv.CurrentRow.Index;
                DialogResult r = MessageBox.Show("هل انت متأكد من رغبتك في حذف الفئة ؟", "تنبيه", MessageBoxButtons.YesNo);
                foreach (DataGridViewRow row in dgv.SelectedRows)
                {
                    if (r == DialogResult.Yes)
                    {
                        Delete(row.Cells[0].Value.ToString());
                        MessageBox.Show("تم مسح الفئة");
                        dgv.RowHeadersWidthSizeMode = DataGridViewRowHeadersWidthSizeMode.DisableResizing; //>vip
                        dgv.ColumnHeadersVisible = false;
                        dgv.DataSource = getCategories();
                        dgv.ColumnHeadersVisible = true;
